...
<div>
<table>
<tr><th>Organizasyon Başkanı<th><th>Organizasyon Tarihi</th><th>Son Başvuru Tarihi</th><th>Turnuva Yeri</th><th>Katıl</th><Ayrıntılar</tr>
<?php
include "bag.php"; 
...
$result = $conn->query($sql);
 while($row = $result->fetch_assoc()) { 
		echo	"<tr><td>".$row["orgBaskan"]."</td><td></td><td>".$row["tar"]."</td><td>".$row["sonBasTar"]."</td><td>".$row["ilAd"]." ".$row["ilceAd"]."</td><td><a href=".$base_url."/kulup/sporcuTurnuva/sporcuTurnuvaEkle.php?tur=".$row["turnuvaId"].">Seç</a></td><td><a href=".$base_url."/Ayrinti.php?id=".$row["turnuvaId"].">Ayrıntı</a></td></tr>";
    }
